...
所有人部署RAG的方式都一样：按512个token分块，用text-embedding-3-small嵌入，top-k=5，塞入上下文。这对演示有效。但一到生产环境：
...
延迟：500ms嵌入 + 200ms向量搜索 + 300ms大模型 = 每查询1秒以上
...
纯向量搜索会遗漏精确匹配（错误码、函数名）。纯BM25会遗漏语义匹配。混合方案胜出。
...
双编码器（嵌入）相似度与相关性相关性≈0.75。交叉编码器≈0.92。50→5的漏斗成本50ms，但提升15%召回率。
...
chunk_size=512, top_k=5, similarity_threshold=0.7——谁选的？我们将检索视为黑盒函数 f(分块大小, 重叠, top_k, 权重) → 召回率@10, 延迟，并用贝叶斯搜索优化。
...
Results: 40% latency reduction, 95% recall@10 across document types.
结果：延迟降低40%，跨文档类型召回率@10达95%。