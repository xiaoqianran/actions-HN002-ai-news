...
originalUrl: "https://dev.to/imus_d7584cbc8ee9b0336256/optimizing-rag-at-scale-chunking-retrieval-and-the-bayesian-search-that-cut-latency-40-2234"
date: "2026-07-20T23:12:24.589Z"
---

# Optimizing RAG at Scale: Chunking, Retrieval, and the Bayesian Search That Cut Latency 40%  
# 大规模优化 RAG：分块、检索与削减延迟 40% 的贝叶斯搜索  

How we moved from "semantic search + hope" to a measured, tunable retrieval pipeline with 95% recall@10  
我们如何从“语义搜索 + 碰运气”转变为可衡量、可调优的检索流水线，实现 95% 的召回率@10  
...
所有人部署 RAG 的方式都一样：按 512 个令牌分块，用 text-embedding-3-small 嵌入，top-k=5，塞入上下文。这适用于演示。  

Then you hit production:  
但进入生产环境后：  

- Legal contracts: 512 tokens splits clauses mid-sentence  
  法律合同：512 令牌的分块会在句子中间拆分条款  
- API docs: 1000-token chunks drown signal in noise  
  API 文档：1000 令牌的分块会让信号淹没在噪声中  
...
纯向量搜索会遗漏精确匹配（错误代码、函数名）。纯 BM25 会遗漏语义匹配。混合方法胜出。  
...
Bi-encoder (embedding) similarity ≈ 0.75 correlation with relevance. Cross-encoder ≈ 0.92. The 50→5 funnel costs 50ms but gains 15% recall.  
双编码器（嵌入）相似度与相关性相关性 ≈ 0.75。交叉编码器 ≈ 0.92。50→5 的漏斗成本为 50 毫秒，但提升 15% 召回率。  
            ...
            messages=[
                {
                    "role": "system",
                    "content": """
                    Generate diverse search queries that collectively cover the user's intent. Include:
                    exact phrasing, synonyms, broader/narrower, hypothetical answer.
                    """
                },
...
`chunk_size=512, top_k=5, similarity_threshold=0.7` —— 谁选的这些？  
...
我们将检索视为黑盒函数 `f(分块大小, 重叠, top_k, 权重) → 召回率@10, 延迟`，并用贝叶斯搜索优化。  
...
Result: 40% lower latency, 95% recall@10 across all document types.  
结果：所有文档类型的延迟降低 40%，召回率@10 达到 95%。  

No more guessing. Tune everything.  
不再猜测。调优一切。